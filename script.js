...
api_key = 'e8c01030baf9a3f13a96edf5acf2ccbc',
...
function getWeatherDetails(name , lat , lon , country , state){
    lat = parseFloat(lat);
    lon = parseFloat(lon);
    let FORECAST_API_URL_ = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${api_key}`,
    WEATHER_API_URL_ = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${api_key}`,
   ...
   fetch(WEATHER_API_URL_).then(res => res.json()).then((data)=>{
    let date = new Date();
   ...
   fetch(FORECAST_API_URL_).then(res => res.json()).then(data =>{
    ...
    for(i=0;i<=7;i++){
       ...
       fiveDaysForecastCard[0].innerHTML = ' ';
...
function getCityCoordinates(){
    ...
    fetch(GEOCODING_API_URL_).then(res => res.json()).then(data => {
      ...
      alert(`Failed to fetch the coordinated of ${cityName}`);
    });
}
...
function getUserCoordinates(){
   ...
   if(error.code === error.PERMISSION_DENIED){
      alert("Permission Allow Karde Bhai");
   }
    }
);
}
